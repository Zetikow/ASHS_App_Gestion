...
// renderAverageCard) ouvre une fiche (bottom sheet) listant les événements
// où il a été absent puis présent sur la même période — voir
...
function computePresenceStreaks(equipe) {
  const roster = rosterForEquipe(equipe);
  const now = new Date();
  const evs = evenements.filter(ev => eventVisibleForTeam(ev, equipe) && eventDateObj(ev) < now)
...
// ===================== SÉLECTION MATCH =====================
// Distinct de la présence : qui est retenu dans l'effectif du match, pas juste disponible.
...
// la fiche (voir renderPresenceSelectionSheet). Une fois publiée, deux effets côté joueuse :
// - elle peut taper la carte du match pour voir qui est retenue (renderSelectionCardButton /
...
//   Non sélectionnée (voir selectionStatusFor, utilisé depuis agenda.js).
...
function renderSelectionSection(activeTeam) {
  const matches = sortedEvenements().filter(ev => eventVisibleForTeam(ev, activeTeam) && typeClass(ev[3]) === "match");
  if (matches.length === 0) {
...
function renderPresenceEventCard(ev, isPast, activeTeam) {
  ...
  const d = eventDateObj(ev);
  const roster = rosterForEquipe(activeTeam || "SF1").map(p => ({ p, val: presenceEvenements[`${id}_${p}`] }));
...
function renderPresenceSelectionSheet(activeTeam) {
  ...
  const dayName = d.toLocaleDateString("fr-FR", { weekday: "long" });
  const displayTitre = formatMatchDisplay(titre, lieu).label || titre || "Match";
  const roster = rosterForEquipe(activeTeam || "SF1");
  const count = selectionCountFor(id);
  const countColor = count >= SELECTION_MAX_PLAYERS ? "#ff5a5a" : "#33d17a";
  const published = selectionIsPublished(id);

  let bodyHtml = `<div class="stat-bar-row">
    <span style="color:${countColor}; font-weight:800;">${count}/${SELECTION_MAX_PLAYERS} sélectionnées</span>
  </div>`;
  if (roster.length === 0) {
  ...
  // Publication : une fois les 12 choisies, le coach rend la sélection visible aux joueurs
  // (bouton toujours accessible, pas seulement à 12/12 — comme la composition, qui n'impose pas
  // non plus d'effectif complet avant publication). Voir selectionPublishApi / api_publishSelection.
...
// ===================== FICHE SÉLECTION — LECTURE SEULE (JOUEUSES/PARENTS) =====================
