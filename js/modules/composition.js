...
// Effectif combiné U17M1 + U17M2 (sans doublon) : les deux équipes s'entraînent ensemble et un
// joueur M2 peut être appelé pour un match M1 (voir le suivi des brûlages, page Présence).
function compositionRoster() {
  const seen = new Set();
  const all = [];
  U17_TEAMS.forEach(eq => rosterForEquipe(eq).forEach(p => { if (!seen.has(p)) { seen.add(p); all.push(p); } }));
  return all;
}
...
function renderCompositionCardButtons(ev) {
  if (!U17_TEAMS.includes(eventEquipe(ev)) || typeClass(ev[3]) !== "match") return "";
  const matchId = ev[0];
