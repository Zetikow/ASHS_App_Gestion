...
// ===================== FICHE SAISIE RAPIDE (bottom sheet) =====================
// Porté depuis Clubly : l'ancien formulaire "+ Saisir une action" inline (add-form) devient une
// fiche qui glisse depuis le bas — voir window.__showQuickAdd, câblé sur #cn-add-action.
function renderQuickAddSheet() {
  ...
  const isRetard = actionLabel === "Retard (prévenir 30min avant)" || actionLabel === "Retard (match)";
...
// ===================== FICHE MODIFIER UNE ACTION (bottom sheet) =====================
// Porté depuis Clubly : l'ancien bloc "cn-edit-picker" affiché inline dans la carte du joueur
// devient une fiche à part — voir window.__cnEditPlayer, câblé sur .cn-modifier-btn
// (data-cn-toggle-edit). Fermeture (croix/scrim) via data-cn-edit-cancel, géré ci-dessous avec
// closeSheetAnimated (même principe que data-close-sheet, mais garde son propre handler car le
// drapeau à annuler n'est pas un simple window.__xxx unique — trois flags à remettre à null).
