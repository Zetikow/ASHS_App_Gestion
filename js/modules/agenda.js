...
function getVisibleEvenements() {
  ...
  if (session && !hasRole("Admin")) {
    ...
    list = list.filter(ev => {
      if (myEquipe === "Toutes") return eventEquipe(ev) === "Toutes";
      return eventVisibleForTeam(ev, myEquipe);
    });
  }
...
function sortedEventsForTeamView(equipeView) {
  ...
  if (equipeView === "Toutes") {
    ...
    list = evenements.filter(ev => eventVisibleForTeam(ev, equipeView));
  }
...
function renderAgenda() {
  ...
  const pageSub = activeTeam === "Toutes"
    ...
    : (activeTeam === "U17" ? "Matchs, entraînements et événements de U17 (M1 et M2)."
      : (U17_TEAMS.includes(activeTeam) ? `Matchs, entraînements et événements ${activeTeam}.` : `Événements ${activeTeam}.`));
  ...
  if (past.length > 0) {
    ...
    if (pastOpen) {
      past.slice(0, 8).forEach(ev => { html += renderEventCard(ev, canManage, true, activeTeam === "U17", staggerIndex++); });
    }
  }
  ...
  if (upcoming.length === 0) {
    ...
    upcoming.forEach(ev => {
      ...
      html += renderEventCard(ev, canManage, false, activeTeam === "U17", staggerIndex++);
    });
  }
...
function renderEventCard(ev, canManage, isPast, showTeamBadge, staggerIndex) {
  ...
  if (canManage && window.__editingEvenementId === id) {
    return `<div class="ev-card" style="display:block;">
      ...
      <select id="edit-ev-equipe-${id}">
        ${TEAMS.map(t => `<option value="${t}" ${(equipe || "SF1") === t ? "selected" : ""}>${t}</option>`).join("")}
...
function renderEventDetailSheet() {
  ...
  if (!isPast && !hasRole("Bénévole") && !sansPresence && !selStatus && presIdentity.editable && !compositionNonSelected(ev, presIdentity.nom)) {
    html += `<div class="row-flex" style="margin-top:8px;">
      <div class="pres-toggle" style="flex:1;">
        <button type="button" class="pres-toggle-option ${val === 'Oui' ? 'active' : ''}" data-event-presence="${id}" data-event-val="1">Présent</button>
        <button type="button" class="pres-toggle-option ${val === 'Non' ? 'active' : ''}" data-event-presence="${id}" data-event-val="0">Absent</button>
      </div>
    </div>`;
...
async function updateEvenementApi(id, date, heure, type, titre, lieu, equipe, score, sansPresence) {
  try {
    const params = new URLSearchParams({ action: "updateEvenement", id, date, heure, type, titre, lieu, equipe: equipe || "SF1", authNom: session.nom, authCode: session.code });
...
function attachAgendaEvents() {
  ...
  document.querySelectorAll("[data-save-event]").forEach(el => {
    el.onclick = () => {
      ...
      const equipe = equipeSelect ? equipeSelect.value : (existing ? existing[6] : "SF1");
      if (!date || !titre) { alert("Merci de renseigner au moins la date complète et le titre."); return; }
      let score;
