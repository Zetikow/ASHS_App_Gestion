...
function visibleActualites() {
  let list = actualites;
  if (!hasRole("Admin") && !hasRole("Salarié")) {
...
function actuFilterOptions() {
  if (hasRole("Admin") || hasRole("Salarié")) return ["Générale", ...TEAMS];
...
// Porté depuis Clubly : l'ancien formulaire "+ Ajouter une actualité" inline (add-form) devient
// une fiche qui glisse depuis le bas — voir window.__showAddActualite, câblé sur
// #toggle-add-actualite, fermée via data-close-sheet="showAddActualite".
...
function attachActualitesEvents() {
  ...
  if (toggleAddActualite) toggleAddActualite.onclick = () => { vibrate(); window.__showAddActualite = true; render(); };
