...
function renderGestionMatchsDetailSheet() {
  ...
  if (section === "covoiturage") {
    ...
    bodyHtml = `<div class="cp-col-h" style="color:var(--accent2);">🍪 Choses apportées (${entries.length})</div>
...
function renderGouterSection(activeTeam) {
  ...
  matches.forEach(ev => {
    ...
    html += `<div class="cp-match-card">` + matchCardHeader(ev, `
      <div class="cp-summary-badge"><div class="num" style="color:var(--accent2);">${signups.length}</div><div class="lbl">Inscrits</div></div>
    `, "gouter");

    html += `<div class="cp-col-h" style="color:var(--accent2);">Qui amène quoi</div>`;
    if (signups.length === 0) {
