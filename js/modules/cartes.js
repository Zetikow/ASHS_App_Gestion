// ===================================================================
// CARTES D'ÉVÉNEMENT — cartes optionnelles attachées à un événement SF1/SF2
...
function renderCartesForEvent(ev, canManage) {
  ...
  const identities = myCarpoolIdentitiesForTeam(eventEquipe(ev));
  let html = "";
  list.forEach(carte => {
...
function renderRestaurantsSection() {
  ...
  if (window.__showAddRestaurant) {
    html += `<div class="add-form">
      ...
      <input id="restaurant-new-nom" type="text" placeholder="Ex: La Table d'Hoenheim" />
