...
const LOGO_DATA_URI = "images/logo.svg";
...
const APP_VERSION = "2026-07-24-27"; // À incrémenter à chaque mise à jour déployée
...
// Un compte multi-rôles (ex: Joueur SF1 + Coach U17 + Admin) cumule automatiquement tous ses
...
function primaryEquipe() {
  if (!session || !Array.isArray(session.roles) || session.roles.length === 0) return "SF1";
  ...
  return TEAMS.includes(firstEquipe) ? firstEquipe : "SF1";
}
function rolesLabel() {
  if (!session || !Array.isArray(session.roles)) return "";
...
function myJoueurIdentity() {
  ...
  if (joueurRole) return { nom: joueurRole.nom || session.nom, equipe: joueurRole.equipe || "SF1" };
  return null;
}
...
let loginCodeLength = 4; // 4 chiffres par défaut, 6 pour un compte Admin (voir checkAccountStatus)
...
// Toute la page Support est gardée (elle a toujours un champ de message ouvert). Voir
...
// Roles(2, ex: "Joueur:SF1,Coach:U17,Admin:Toutes"), Poste(3), NomComplet(4), PhotoURL(5),
...
function parseRoles(cell) {
  return String(cell || "").split(",").map(s => s.trim()).filter(Boolean).map(pair => {
    ...
    if (idx === -1) return { role: pair, equipe: "SF1" };
    return { role: pair.slice(0, idx).trim(), equipe: pair.slice(idx + 1).trim() || "SF1" };
  });
}
...
// SF1 qui est aussi coach U17 voit les deux), et toutes pour l'Admin.
