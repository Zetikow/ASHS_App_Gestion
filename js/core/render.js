...
function renderLogin() {
  ...
  return `<div class="login-wrap"><div class="login-card-outer"><div class="login-card">
    ...
    <div class="login-title">ASHS</div>
...
// jours" comme getCurrentWeekEvents — ici on veut aussi les événements déjà passés cette
// semaine, pour le récap).
...
function renderHome() {
  ...
  const homeActus = visibleActualites().slice(0, 2);
...
function render() {
  ...
  let html = `<div class="header ${currentPage === 'home' ? 'home-header' : ''}">
    <img src="${LOGO_DATA_URI}" alt="Logo ASHS"/>
    <div class="hdr-textwrap"><div class="title">ASHS</div><div class="subtitle">App de Gestion</div></div>
  ...
  // il vivait avant : c'était un bug, le bouton ne faisait rien depuis l'Accueil), il faut que
  // l'éditeur puisse s'ouvrir peu importe la page active. Anciennement câblé uniquement dans
  // renderAgenda() (js/modules/agenda.js) — corrigé en le remontant ici (porté depuis Clubly).
