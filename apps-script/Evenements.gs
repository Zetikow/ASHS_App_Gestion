...
// vide sinon) — utile pour un match visible de tout le club où seul le suivi foodtruck compte,
// sans qu'on demande à tout le monde de pointer présent/absent.
...
// et marque "SF1" par défaut les événements déjà créés (tous étaient SF1 jusqu'ici).
function ensureEvenementsEquipeColumn(sheet) {
  ...
  for (let i = 1; i < data.length; i++) {
    if (data[i][0] && !data[i][6]) {
      sheet.getRange(i + 1, 7).setValue("SF1");
    }
  }
}
...
function api_generateSeasonTrainings(ss, e) {
  ...
  const sheet = ss.getSheetByName("Evenements");
  const data = sheet.getDataRange().getValues();
  const equipe = e.parameter.equipe || "SF1";
  const existingKeys = new Set();
  for (let i = 1; i < data.length; i++) {
    if (data[i][3] === "Entraînement") existingKeys.add(`${data[i][1]}_${data[i][6] || "SF1"}`);
  }
...
function api_addEvenement(ss, e) {
  ...
  sheet.getRange(row, 7).setValue(e.parameter.equipe || equipesCoach[0] || "SF1");
  ...
  for (let i = 1; i < data.length; i++) {
    if (data[i][0] === id) {
      ...
      if (Object.prototype.hasOwnProperty.call(e.parameter, "equipe")) {
        sheet.getRange(row, 7).setValue(e.parameter.equipe || "SF1");
      }
...
function notifyEvenementUpdatedPush(ss, before, newDate, newHeure, newLieu, newType, newEquipeParam) {
  try {
    ...
    const equipe = newEquipeParam !== undefined ? (newEquipeParam || "SF1") : (before[6] || "SF1");
