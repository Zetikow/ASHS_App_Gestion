...
//   FCM_PROJECT_ID    : "appgestionashs" (l'ID du projet Firebase — À
//                       COMPLÉTER avec l'ID réel une fois le projet créé)
...
// envoyé avec succès — même logique que l'envoi de mail en masse (voir sendDisponibilitesReminders
// dans Notifications.gs), à brancher progressivement pour chaque type de rappel de la liste.
...
// fichiers .gs (Compositions, Selections, Grid, Evenements, Actualites, Support,
...
// utile pour "Admin"... (rôles rattachés à la personne plutôt qu'à une équipe précise).
...
// enregistré (pas de nom à deviner).
function testPushNotification() {
  ...
  for (let i = 1; i < data.length; i++) {
    const token = data[i][COL_PUSHSUBIDS];
    if (token) {
      const ok = sendPushNotification(token, "Test ASHS", "Si tu vois ceci, les notifications push fonctionnent !");
      Logger.log(ok
...
function testPushToPerson() {
  ...
  for (let i = 1; i < data.length; i++) {
    if (String(data[i][COL_NOM]).trim() === nomCible) {
      ...
      const ok = sendPushNotification(token, "Test ASHS", "Ceci est un test envoyé spécifiquement à toi !");
