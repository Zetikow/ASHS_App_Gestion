...
// équipe touche les Joueur/Coach/Parent de cette équipe, PLUS Admin/Salarié qui voient
...
function notifyActualitePush(ss, titre, scope) {
  try {
    const body = `"${titre}" vient d'être publiée sur ASHS.`;
    let tokens;
    if (scope === "Générale") {
      tokens = pushTokensAll(ss);
    } else {
      const teamTokens = pushTokensForEquipe(ss, scope, ["Joueur", "Coach"], true);
      const broadTokens = [].concat(pushTokensForRole(ss, "Admin"), pushTokensForRole(ss, "Salarié"));
