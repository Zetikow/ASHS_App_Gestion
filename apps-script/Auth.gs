...
// "Joueur:SF1,Coach:U17,Admin:Toutes" — plus besoin de plusieurs lignes reliées par "Personne".
...
function setupComptes() {
  ...
  if (sheet.getDataRange().getNumRows() <= 1) {
    ...
    const rows = PLAYERS.map(p => [p, "", "Joueur:SF1", "", "", "", "", ""]);
    rows.push(["Coach", "", "Coach:SF1", "COACH", "", "", "", ""]);
    rows.push(["Admin", "", "Admin:Toutes", "", "", "", "", ""]);
    rows.push(["Bénévole", "", "Bénévole:SF1", "", "", "", "", ""]);
...
function migrateComptesToSingleRowPerPerson() {
  ...
  order.forEach(personne => {
    ...
    rows.forEach(row => {
      ...
      const equipe = String(row[3] || "SF1").trim();
