...
function api_notifyPaymentClaim(ss, e) {
  ...
  const body = nom + " indique avoir réglé sa cotisation (" + montant + " €).\n\n"
    + "Merci de vérifier la réception du paiement puis de l'enregistrer dans ASHS (page Paiements → \"+ Ajouter un paiement\").";
  [...new Set(adminEmails)].forEach(em => {
    try { MailApp.sendEmail(em, "ASHS — Paiement à valider : " + nom, body, { name: "ASHS — Paiements" }); } catch (err) { /* pas bloquant */ }
  });
  return jsonOut({ ok: true });
}
