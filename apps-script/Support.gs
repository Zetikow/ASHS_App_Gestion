...
function api_sendSupportMessage(ss, e) {
  ...
  const body = "Nouveau message depuis ASHS :\n\n"
  ...
  try {
    const mailOptions = { name: "ASHS — Support" };
    if (expediteurEmail) mailOptions.replyTo = expediteurEmail;
    MailApp.sendEmail(CLUB_SUPPORT_EMAIL, "ASHS — Question de " + e.parameter.authNom, body, mailOptions);
    ...
    [...new Set(adminEmails)].filter(em => em !== CLUB_SUPPORT_EMAIL).forEach(em => {
      try { MailApp.sendEmail(em, "ASHS — Question de " + e.parameter.authNom, body, mailOptions); } catch (err) { /* pas bloquant */ }
    });
  } catch (err) {
...
function api_replySupportMessage(ss, e) {
  ...
  for (let i = 1; i < data.length; i++) {
    if (data[i][0] === id) {
      ...
      if (auteurEmail) {
        try {
          MailApp.sendEmail(auteurEmail, "ASHS — Réponse à ta question",
